...
import { TEST_CONFIG } from "./utils/api-test-helpers";
import { mockApiResponse } from "./utils/transaction-test-helpers";
...
const mockP24Api = async (page: Page) => {
  await page.route("**/sandbox.przelewy24.pl/**", async (route) => {
    await route.fulfill({
      status: 502,
      contentType: "application/json",
...
test.describe("Transaction Status API", () => {
  const { baseURL, apiEndpoint } = TEST_CONFIG("/api/transaction/start");
  ...
  test.describe("Transaction not found scenarios", () => {
    test("should return 404 when transaction is not found for authenticated user", async ({
      request,
    }) => {
      ...
      const response = await request.get(
        `${baseURL}${apiEndpoint}/non-existent-id`
      );
      ...
      if (status === 200) {
        console.log(
          "Server returned 200 - might not be running or endpoint not found"
        );
  ...
  test.describe("P24 API failure scenarios", () => {
    test("should return 502 when P24 API is unreachable", async ({
      request,
      page,
    }) => {

        await mockP24Api(page);
        ...
        const body = await response.json();
        expect(body.message).toBe("Failed to create transaction");
  ...
  test.describe("Successful scenarios", () => {
    test("should return 200 with status when transaction is found and P24 API is successful", async ({
      request,
    }) => {
  ...
  test.describe("Edge cases", () => {
    test("should handle malformed session ID gracefully", async ({
      request,
    }) => {
      const response = await request.get(
        `${baseURL}${apiEndpoint}/malformed-session-id`
      );
    ...
    test("should handle very long session ID gracefully", async ({
      request,
    }) => {
...
test.describe("Integration scenarios", () => {
  test("should work with real authentication and database", async ({
    request,
  }) => {
