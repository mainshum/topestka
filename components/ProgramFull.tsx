...
const ProgramFull = ({ accordions }: { accordions: AccordionItemData[] }) => {
  return (
    ...
    >
      ...
      <p className="pl-1 relative font-monarcha text-electric-600 text-xl lg:text-3xl">
          ...
          <Accordion key={title} type="single" collapsible>
            <AccordionItem value={title}>
              <AccordionTrigger
                ...
                className="pb-4 px-4 border-b border-b-eblue-300 font-medium text-eblue-600 font-outfit text-xl lg:text-4xl transition max-md:duration-500 data-[state=open]:rounded transition-discrete data-[state=open]:bg-[#2A4BCC] data-[state=open]:opacity-85 data-[state=open]:text-butter-100"
              >
                {title}
              </AccordionTrigger>
              <AccordionContent className="px-2 pb-0 lg:px-4 py-2 lg:pt-4 font-extralight text-xl lg:text-3xl ">
                {content}
              </AccordionContent>
            </AccordionItem>
          </Accordion>
