...
>(({ className, children, chevProps, ...props  }, ref) => {
  ...
  <AccordionPrimitive.Header className="flex">
    <AccordionPrimitive.Trigger
      ref={ref}
      className={cn(
        "flex flex-1 items-center justify-between py-3 text-lg font-normal transition-all duration-1000 text-left [&[data-state=open]>svg]:rotate-180",
        className
      )}
      {...props}
    >
      {children}
      <ChevronDown className={cn("w-5 h-5 text-muted-foreground text-orange-400 transition-transform duration-1000 shrink-0", chevClassName )} {...rest}/>
    </AccordionPrimitive.Trigger>
